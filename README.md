...
Після `/start` новий користувач отримає опис курсу з кнопкою `Реквізити для оплати`. Після натискання кнопки бот надсилає реквізити, призначення платежу у форматі Telegram code для зручного копіювання і прохання надіслати скріншот квитанції. Коли користувач надсилає фото, бот відправляє його в групу `MANUAL_PAYMENT_REVIEW_CHAT_ID` з кнопками `Підтвердити` і `Відхилити`.
